...
public class NotificationController {
    ...
    public ResponseEntity<NotificationEvent> createNewNotificationEvent(@RequestBody NotificationEvent notificationEvent) throws JsonProcessingException, ExecutionException, InterruptedException {

        RegisteredClient client = clientRepository.findByClientId(notificationEvent.getClientId());

        if(client == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(notificationEvent);
        }

        String topic = client.getTopicName();
        notificationEventProducer.sendNotificationEvent(notificationEvent, topic);
        return ResponseEntity.status(HttpStatus.CREATED).body(notificationEvent);
    }
}
